(function () {
  ...
  const COPY = {
    zh: {
      ...
      introRule3: "黄色胸前标记只表示躯干正面；球形头部不提供视线方向。不要据此评价精细头部朝向。",
      reviewerId: "评审代号",
      ...
      sharedFocus3: "黄色标记表示躯干正面；球形头部不表达精细视线方向，无法判断时不要猜测。",
    ...
    en: {
      ...
      introRule3: "The yellow chest marker indicates only the torso front. The spherical head does not encode gaze direction, so do not use it to judge fine head orientation.",
      ...
      sharedFocus3: "The yellow marker indicates the torso front; the spherical head does not encode fine gaze direction, so do not guess.",
