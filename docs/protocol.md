...
E1c 将动作—轨迹耦合单独评估。主条件为 GEM 原生 global common-17、经过一次整段旋转/尺度适配后放置在相同 GEM root trajectory 上的 MotionBERT，以及只将 GEM root-relative articulation 错开 15 帧的负对照。负对照不循环、不补帧，只裁剪共同区间；另以 ±8、±15、±30 帧检查剂量响应。人工材料必须使用相同参考视频、低面数体积人偶、左右肢体配色、世界固定棋盘地面、投影和轨迹视图；每段片段依据完整共享轨迹只确定一次正交相机范围和方位，播放期间不跟随人物、不平移、不缩放。若单目全局重建的任意世界规范使头部位于 pelvis 下方且脚踝位于其上方，则只对整段使用一次绕 x 轴 180° 的 proper rotation 统一为 y-up；不得逐帧翻转或改变骨长。推断触地脚以同色圆环提示。MotionBERT shared-root 是外部轨迹组装控制，不得写成 MotionBERT 原生轨迹输出；common-17 脚接触指标和触地圆环是诊断，不是真值物理接触误差。

GEM 与 MotionBERT 在人工材料中都先映射为完全相同的 common-17 显示拓扑，因此不会因可见头部关节数量不同而直接偏向某一种方法。但 common-17 的头部只有 neck/head center，球形头部也没有朝向标记；它只能支持头部整体位移和躯干动作的粗粒度观察，不能公平评价 head yaw、视线方向以及 LOS/FTT 的精细朝向。此类 cue 在当前 E1c 盲评中标为不可评价，若后续需要研究，应另建两者共同支持的头部朝向表示或单列 native-capability 实验。

E1d 单列为 E1c 之后的原生端到端输出评价，不与前两部分穿插。GEM 条件保留原生 SMPL-X 表面与原生 global root trajectory；MotionBERT 条件保留17关节 root-relative motion，不为其人工补轨迹或蒙皮。为保证画面可读，MotionBERT 只允许使用一次整段刚体显示对齐和身体尺度适配，禁止逐帧适配。两侧使用相同参考片段、固定观察方向、播放速度和匿名 A/B 顺序。人工评价仅比较可见交互保真度与身体/头部朝向可读性；“能否直接进入 Motion Pool”由两种输出的数据结构预先决定，不作为人工主观问题。

E1d 有意保留方法原生表示能力的不对等，因此不是严格双盲的纯姿态精度实验：评审可能从网格、骨架和轨迹看出方法类别。它只能支持“完整输出与下游成本”的系统级结论，不能单独证明 GEM 的局部动作更准确；局部动作质量仍以 E1c 的共同表示对照为准。当前本地原始 SMPL-X 仅覆盖5段 MegaSAM+GEM 结果，故先标为 pilot，后续若扩展样本必须直接复用同一渲染与问卷协议。

人工材料预审排除 `39_gp_set_0001_vid_0003_gp_250_HG_NIGHT_CROSS_unknown_medium`。该段 GEM common-17 的身体尺度为批次明显离群值，并伴随异常后仰与脚部漂移；MotionBERT shared-root 又会继承以该异常 GEM 序列为目标的整段对齐，因此两侧均不再支持有效的动作或耦合判断。此片仍保留在原始自动评测和失败案例记录中，但不进入人工问卷，也不以人工拉直、压低或改骨长的方式修饰。
