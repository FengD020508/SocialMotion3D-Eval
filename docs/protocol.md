...
E1d 单列为 E1c 之后的原生端到端输出评价，不与前两部分穿插。GEM 条件保留原生 SMPL-X 表面与原生 global root trajectory；MotionBERT 条件保留17关节 root-relative motion，不为其人工补轨迹或蒙皮。为保证画面可读，MotionBERT 只允许使用一次整段刚体显示对齐和身体尺度适配，禁止逐帧适配。两侧使用相同参考片段、固定观察方向、播放速度和匿名 A/B 顺序。E1d 评价可见交互保真度、身体/头部朝向可读性，以及在不人工补轨迹或重新绑定人体时对三维场景与 Motion Pool 的直接使用价值。
