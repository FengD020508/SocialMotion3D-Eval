...
def main() -> None:
    ...
    if raw_out.exists():
        raw_out.unlink()
    run([str(MEGASAM_PYTHON), "camera_tracking_scripts/test_demo.py", "--datapath", str(frames),
         "--weights", str(mega_ckpt), "--scene_name", scene,
         "--mono_depth_path", str(mono_root), "--metric_depth_path", str(metric_root), "--disable_vis"])
    ...
    if len(T_raw) != n:
        raise RuntimeError(f"MegaSAM pose length {len(T_raw)} != video length {n}")
    ...
    if K[0, 0] <= 0 or K[1, 1] <= 0:
        raise RuntimeError(f"MegaSAM returned non-positive focal length after conversion: {K}")
