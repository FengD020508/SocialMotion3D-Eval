...
"""
GEM-SMPL demo: text-only generation or mixed video + text conditioning.

Usage:
    # Text only (each segment uses --text_length):
    python scripts/demo/demo_smpl.py \
        --input_list "text:a person walks" "text:the person waves" \
        --text_length 120 --ckpt_path path/to/gem_smpl.ckpt

    # Text only, with a different length for each text segment:
    python scripts/demo/demo_smpl.py \
        --input_list "text:a person walks" "text:the person waves" \
        --text_lengths 120 60 --ckpt_path path/to/gem_smpl.ckpt
...
def assign_text_lengths(
    segments: list[dict],
    default_length: int,
    text_lengths: list[int] | None,
) -> None:
    """Assign a frame length to every text segment in input order."""
    num_text = sum(s["type"] == "text" for s in segments)
    if default_length <= 0:
        raise ValueError("--text_length must be a positive integer.")
    if text_lengths is not None:
        if len(text_lengths) != num_text:
            raise ValueError(
                f"--text_lengths expects one value per text segment "
                f"({num_text} expected, {len(text_lengths)} provided)."
            )
        if any(length <= 0 for length in text_lengths):
            raise ValueError("Every value in --text_lengths must be positive.")

    lengths = iter(text_lengths or [default_length] * num_text)
    for segment in segments:
        if segment["type"] == "text":
            segment["length"] = next(lengths)
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=300,
        help="Default frames per text segment (default: 300 = 10s at 30fps)",
    )
    parser.add_argument(
        "--text_lengths",
        type=int,
        ...
        default=None,
        metavar="N",
        help="Frames for each text segment in input order; overrides --text_length",
    )
    parser.add_argument(
        "--text_width",
        type=int,
        default=1000,
        help="Virtual image width for text-only generation (default: 1000)",
    )
    parser.add_argument(
        "--text_height",
        type=int,
        default=1000,
        help="Virtual image height for text-only generation (default: 1000)",
    )
    parser.add_argument(
        "--text_fov",
        type=float,
        default=43.3,
        help="Virtual camera horizontal FOV for text-only generation (default: 43.3)",
    )
    parser.add_argument(
        "--no_render",
        action="store_true",
    ...
    assign_text_lengths(input_segments, args.text_length, args.text_lengths)
    has_video = any(s["type"] == "video" for s in input_segments)

    # Derive output name from the first video, or use a stable text-only name.
    if has_video:
    ...
    print(
        f"  Text lens:  "
        f"{[s['length'] for s in input_segments if s['type'] == 'text']} frames"
    )
    print("=" * 60)
    ...
    # Text-only generation uses the same default virtual camera as the
    # HumanML3D pure-motion training pipeline.
    if not has_video:
        if args.text_width <= 0 or args.text_height <= 0:
            raise ValueError("--text_width and --text_height must be positive.")
        if not 0 < args.text_fov < 180:
            raise ValueError("--text_fov must be between 0 and 180 degrees.")
        from gem.utils.cam_utils import create_camera_sensor

        ref_width, ref_height, ref_K = create_camera_sensor(
            args.text_width, args.text_height, args.text_fov
        )
        print(
            f"\n  Using virtual camera for text-only generation: "
            f"{ref_width}x{ref_height}, FOV={args.text_fov:g} degrees"
        )
    ...
    for i, seg in enumerate(processed_segments):
        if isinstance(seg, dict) and seg.get("type") == "text" and "K_fullimg" not in seg:
            ...
            processed_segments[i] = create_text_segment(
                seg["caption"], seg["length"], ref_K
            )
