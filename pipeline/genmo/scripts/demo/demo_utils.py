...
def detect_and_track(
    ...
    print(f"[Stage 1a] YOLO device: {'cuda:0' if yolo_device == 0 else 'cpu'}")
    ...
    for start in tqdm(range(0, len(frames), batch_size), desc="YOLO detect", leave=False):
        frame_batch = [
            frame[..., ::-1].copy() for frame in frames[start : start + batch_size]
        ]
        batch_results = model(
            frame_batch,
            classes=[0],
        ...
        for offset, r in enumerate(batch_results):
            i = start + offset
            for box in r.boxes:
    ...
    cache_tmp_path = f"{cache_path}.tmp"
    torch.save(bbx_xys, cache_tmp_path)
...
def load_camera_trajectory_npz(
    camera_path: str,
    expected_length: int,
    width: int,
    height: int,
):
    """Load the IDD-PeD camera trajectory contract used by the SMPL demo.

    ``R_w2c`` and its angular velocity condition GENMO. Translation is loaded
    and returned for bookkeeping/export, but the current checkpoint does not
    receive translation velocity as a network feature.
    """
    from gem.utils.geo_transform import compute_cam_angvel, compute_cam_tvel

    path = Path(camera_path)
    if not path.is_file():
        raise FileNotFoundError(f"Camera trajectory not found: {path}")
    with np.load(path, allow_pickle=False) as z:
        required = {"T_w2c", "T_c2w"}
        missing = sorted(required.difference(z.files))
        if missing:
            raise ValueError(f"{path} is missing required arrays: {missing}")
        T_w2c = torch.from_numpy(np.asarray(z["T_w2c"], dtype=np.float32))
        T_c2w = torch.from_numpy(np.asarray(z["T_c2w"], dtype=np.float32))
        if "K_fullimg" in z.files:
            K_fullimg = torch.from_numpy(np.asarray(z["K_fullimg"], dtype=np.float32))
        elif "intrinsics" in z.files:
            intr = np.asarray(z["intrinsics"], dtype=np.float32)
            if intr.ndim == 1:
                intr = np.repeat(intr[None], len(T_w2c), axis=0)
            K = np.zeros((len(intr), 3, 3), dtype=np.float32)
            K[:, 0, 0], K[:, 1, 1] = intr[:, 0], intr[:, 1]
            K[:, 0, 2], K[:, 1, 2] = intr[:, 2], intr[:, 3]
            K[:, 2, 2] = 1.0
            K_fullimg = torch.from_numpy(K)
        else:
            raise ValueError(f"{path} has neither K_fullimg nor intrinsics")

    for name, value in (("T_w2c", T_w2c), ("T_c2w", T_c2w)):
        if value.shape != (expected_length, 4, 4):
            raise ValueError(
                f"{name} shape {tuple(value.shape)} does not match video length "
                f"{expected_length}"
            )
        if not torch.isfinite(value).all():
            raise ValueError(f"{name} contains non-finite values")
    if K_fullimg.ndim == 2:
        K_fullimg = K_fullimg[None].repeat(expected_length, 1, 1)
    if K_fullimg.shape != (expected_length, 3, 3):
        raise ValueError(f"K_fullimg has invalid shape {tuple(K_fullimg.shape)}")

    R_w2c = T_w2c[:, :3, :3]
    det = torch.linalg.det(R_w2c)
    if (det - 1).abs().max().item() > 1e-3:
        raise ValueError(f"Camera rotations are not SO(3); max |det-1|={float((det-1).abs().max()):.3g}")
    identity_error = (T_c2w[0] - torch.eye(4)).abs().max().item()
    if identity_error > 1e-3:
        raise ValueError(f"First T_c2w is not identity (max error {identity_error:.3g})")

    cam_angvel = compute_cam_angvel(R_w2c, padding_last=True)
    cam_tvel = compute_cam_tvel(T_w2c[:, :3, 3])
    return R_w2c, cam_angvel, cam_tvel, K_fullimg, T_w2c, T_c2w
